...
{
  WorkQueue wq;
  wq.start(std::thread::hardware_concurrency());
  std::stringstream ss;
  if(X.size() == 0)
  {
  ...
  else
  {
    ...
    std::vector<int> scoreL;
    std::condition_variable cv;
    wq.post([&](){
      scoreL = dp_align_opt(lX,Y);
      count += 1;
      if(count.load() == 2) cv.notify_all();
    });
    std::vector<int> scoreR;
    wq.post([&](){
      scoreR = dp_align_opt(rX, rY);
      count += 1;
      if(count.load() == 2) cv.notify_all();
    });
